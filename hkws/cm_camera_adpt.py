...
class CameraAdapter(BaseAdapter):
    ...
    def start_preview(self, cbFunc, userId=0):
        ...
        self.lRealPlayHandle = self.call_cpp("NET_DVR_RealPlay_V40", userId, struPlayInfo, cbFunc, None)
    ...
    def callback_real_data(self, lRealPlayHandle, cbFunc, dwUser):
        print("3333333333333333333", cbFunc)
    ...
    def callback_Dec(self, lRealPlayHandle, cbFunc, dwUser):
        result = self.call_cpp("PlayM4_SetDecCallBack", lRealPlayHandle, cbFunc, dwUser)
        if not result:
            self.print_error("NET_DVR_SetRealDataCallBack 注册回调函数，捕获实时码流数据失败: the error code is ")
        return result
